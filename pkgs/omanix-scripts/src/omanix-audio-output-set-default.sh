...
# omanix:summary=Set the default audio output and move active streams
# omanix:args=<node-id> <sink-name>
# omanix:examples=omanix-audio-output-set-default 42 alsa_output.pci-0000_00_1f.3.analog-stereo

node_id=${1:-}
sink_name=${2:-}

if [[ -z $node_id || -z $sink_name ]]; then
  echo "Usage: omanix-audio-output-set-default <node-id> <sink-name>" >&2
  exit 1
fi
