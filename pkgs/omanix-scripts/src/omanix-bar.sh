...
source omanix-shell-config

usage() {
  cat <<USAGE
Usage: omanix bar <command> [args...]

  use <id>                             Use a bar option as the active bar
  reset                                Return to the built-in Omanix bar
...
Enable and disable widgets with 'omanix plugin enable' and
'omanix plugin disable'.
...
Examples:
  omanix bar use local.neon-bar
  omanix bar put omanix.keyboard-layout --after omanix.clock
  omanix bar move omanix.media left
  omanix bar move omanix.clock --section center --index 0
  omanix bar set omanix.clock format HH:mm
USAGE
}

# ------------------------------------------------------------------ validation

# The plugin catalog (omanix-plugin-catalog) is not yet available; skip
# validation when it is absent and let the shell reject unknown widgets.
bar_option_exists() {
  command -v omanix-plugin-catalog >/dev/null 2>&1 || return 0
  omanix-plugin-catalog | jq -e --arg id "$1" '
...
cmd_use() {
  ...
  bar_option_exists "$plugin" || fail "$plugin is not a known bar option; run 'omanix plugin list'"

  if [[ $plugin == "omanix.bar" ]]; then
...
cmd_defaults() {
  ...
  [[ -n $DEFAULTS_FILE && -f $DEFAULTS_FILE ]] || fail "no declared bar defaults available"

  local optional_widgets="[]"
  local service widget
  for service in dropbox tailscale; do
    command -v "omanix-installed-service-$service" >/dev/null 2>&1 || continue
    if "omanix-installed-service-$service"; then
      widget=$(jq -cn \
        --arg id "omanix.$service" \
        --arg section "$(bar_widget_default_section "omanix.$service")" \
  ...
  commit "$NORMALIZE
    ...
    | def entry_id: if type == \"object\" then (.id // \"\" | tostring) else tostring end;
      def anchor_for(\$section): { left: \"omanix.workspaces\", center: \"omanix.weather\", right: \"omanix.tray\" }[\$section];
  ...
  echo "Restored the default Omanix bar"
}
...
bar_widget_default_section() {
  local catalog
  catalog=$(omanix-plugin-catalog 2>/dev/null) || {
    echo "center"
    return 0
  }
  jq -r --arg id "$1" '
...
# absent shell is carried on from: omanix-migrate marks a 0 return as done.
PUT_RESULT=""
SHELL_ANSWERED=0
ask_to_put() {
  local id="$1" placement="$2" attempt absent=0
  for (( attempt = 0; attempt < ${OMANIX_SHELL_READY_ATTEMPTS:-50}; attempt++ )); do
    if PUT_RESULT=$(omanix-shell shell putBarWidget "$id" "$placement" 2>&1); then
      SHELL_ANSWERED=1
      ...
      if (( SHELL_ANSWERED )); then
        fail "omanix-shell did not become ready; $id was not put on the bar"
      fi
      # A shell being spawned has no socket yet, and nothing says a launch is
      # under way, so give one a few seconds to turn up.
      if (( ++absent >= ${OMANIX_SHELL_ABSENT_ATTEMPTS:-30} )); then
        echo "omanix-shell is not running; $id was not put on the bar" >&2
        return 1
      fi
    else
...
cmd_put() {
  ...
  [[ $PUT_RESULT != "unknown" ]] || fail "$id is not a known widget; run 'omanix plugin list'"
...
cmd_move() {
  ...
  local result
  result=$(omanix-shell shell moveBarWidget "$id" "$(placement_json)")
...
cmd_set() {
  ...
  local result
  result=$(omanix-shell shell setBarWidget "$id" "$key" "$value_json" "$(placement_json)")
  [[ $result == "ok" ]] || fail "$result"
  echo "Set $key on $id"
}
