...
# omanix:summary=Show or set the location used for weather reports
# omanix:args=[--set <name> [lat,lon]|--clear]
# omanix:examples=omanix-weather-location --set "Malibu" 34.03,-118.68 | omanix-weather-location --clear

# State lives in ~/.local/state/omanix/settings/weather.json as
...
LOC_FILE="$HOME/.local/state/omanix/settings/weather.json"
...
"")
  ...
  ;;
--set)
  [[ -n ${2:-} ]] || { echo "Usage: omanix-weather-location --set <name> [lat,lon]" >&2; exit 1; }
  if [[ -n ${3:-} ]]; then
...
--clear)
  ...
  ;;
*)
  echo "Usage: omanix-weather-location [--set <name> [lat,lon]|--clear]" >&2
  exit 1
  ;;
esac
