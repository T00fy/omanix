...
# omanix:summary=Check that a git URL names a repository, not a transport helper
# omanix:args=<git-url>
...
if [[ $url =~ ^([A-Za-z0-9][A-Za-z0-9+.-]*):// ]]; then
  ...
  fail "'$url' names the '$scheme' transport, which Omanix does not clone from."
fi
