...
# Gates webcam features. Delegates to omanix-capture-webcam-list (Q4-05) when
# present; until that lands, falls back to a V4L2 sysfs presence check. Exits 0
# when a webcam is found, 1 otherwise.
if command -v omanix-capture-webcam-list >/dev/null 2>&1; then
  [ -n "$(omanix-capture-webcam-list 2>/dev/null)" ] && exit 0
  exit 1
fi

# TODO(Q4-05): replace this fallback once omanix-capture-webcam-list ships.
