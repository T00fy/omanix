...
# Choose a legible transparent bar text color by sampling the wallpaper under
# the bar and picking whichever of the two candidate colors has more contrast.
...
cmd_present magick || fallback

if [[ -z $background_path ]]; then
  background_path=$(readlink -f "$HOME/.local/state/omanix/current/background" 2>/dev/null || true)
fi
[[ -f $background_path ]] || fallback

if [[ -z $screen_size ]]; then
  if cmd_present hyprctl && cmd_present jq; then
