...
# omanix:summary=Set the default audio input and move active streams
# omanix:args=<node-id> <source-name>
# omanix:examples=omanix-audio-input-set-default 43 alsa_input.pci-0000_00_1f.3.analog-stereo

node_id=${1:-}
source_name=${2:-}

if [[ -z $node_id || -z $source_name ]]; then
  echo "Usage: omanix-audio-input-set-default <node-id> <source-name>" >&2
  exit 1
fi
