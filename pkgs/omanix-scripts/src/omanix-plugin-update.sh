...
# omanix:summary=Update installed git-managed plugins
# omanix:args=[id] [--yes]
...
update_one() {
  ...
  if ! git -C "$dir" fetch --quiet origin HEAD; then
    echo "omanix-plugin-update: fetch failed for '$id'" >&2
    return 1
  fi
  ...
  if (( ! ASSUME_YES )); then
    ...
    if command -v delta >/dev/null 2>&1; then
      ...
      git -C "$dir" --no-pager diff HEAD FETCH_HEAD
    fi
    echo
    confirm "Update $id?" || {
      echo "Skipped $id."
  ...
  if ! git -C "$dir" merge --ff-only FETCH_HEAD >/dev/null 2>&1; then
    echo "omanix-plugin-update: cannot fast-forward '$id'; you have local changes in $dir" >&2
    return 1
  fi

  if ! omanix-plugin-validate "$dir"; then
    git -C "$dir" reset --hard ORIG_HEAD >/dev/null
    echo "omanix-plugin-update: update of '$id' failed validation; rolled back" >&2
    return 1
  fi
...
while (( $# > 0 )); do
  ...
  --yes | -y)
    ...
    ;;
  -h | --help)
    echo "Usage: omanix-plugin-update [id] [--yes]"
    exit 0
    ;;
  -*)
