...
# omanix:summary=Transcode pictures and videos for sharing
# omanix:args=[--path path] [input] [format] [resolution]
# omanix:examples=omanix-transcode|omanix-transcode --path ~/Downloads|omanix-transcode ~/Videos/demo.mov mp4 1080p|omanix-transcode ~/Pictures/background.heic jpg medium
...
usage() {
  cat <<'EOF'
Usage:
  omanix-transcode [--path path] [input] [format] [resolution]
...
# omanix's menu is the omanix.menu shell plugin driven over IPC via
# omanix-menu-dmenu (stdin -> picked line on stdout), replacing omarchy's
# omarchy-menu-select / omarchy-menu-file.
menu_select() {
  local prompt="$1"
  shift
  printf '%s\n' "$@" | omanix-menu-dmenu -p "$prompt"
}

menu_file() {
  local prompt="$1" search_path="$2" exts="$3"
  local dirs=() find_args=() ext first=1

  IFS=':' read -ra raw <<<"$search_path"
  for d in "${raw[@]}"; do
    [[ -d $d ]] && dirs+=("$d")
  done
  ((${#dirs[@]} > 0)) || return 1
  ...
  find "${dirs[@]}" -type f \( "${find_args[@]}" \) 2>/dev/null | sort | menu_select "$prompt"
}
...
main() {
  ...
  if [[ -z $input ]]; then
    input=$(menu_file "Transcode picture or video" "$search_path" "jpg jpeg png webp gif heic avif mp4 mov m4v mkv webm avi")
  fi
  ...
  if [[ -z $format ]]; then
    if [[ $type == "picture" ]]; then
      format=$(menu_select "Select format" jpg png)
    else
      format=$(menu_select "Select format" mp4 gif)
    fi
  fi

  if [[ -z $resolution ]]; then
    if [[ $type == "picture" ]]; then
      resolution=$(menu_select "Select resolution" high medium low)
    else
      resolution=$(menu_select "Select resolution" 4k 1080p 720p)
    fi
  fi
