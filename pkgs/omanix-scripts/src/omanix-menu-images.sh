...
# omanix:summary=Open a generic image selector menu
# omanix:args=[--selected <image>] [--print-name] [--show-labels] [--filterable] [--lazy-thumbnails] [--preload] [--cache-only] <image-dir>...
...
usage() {
  echo "Usage: omanix-menu-images [--selected <image>] [--print-name] [--show-labels] [--filterable] [--lazy-thumbnails] [--preload] [--cache-only] <image-dir>..."
}
...
cache_dir=${XDG_CACHE_HOME:-$HOME/.cache}/omanix/image-selector
...
if [[ $preload == true ]]; then
  omanix-shell image-selector preload "$rows_b64" "$selected_list_image" "$show_labels" "$filterable" >/dev/null || true
  exit 0
fi

if ! open_result=$(omanix-shell image-selector open \
     "" \
     "$rows_b64" \
     "$selected_list_image" \
     "$selection_file" \
