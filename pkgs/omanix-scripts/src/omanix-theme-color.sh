...
# omanix:summary=Resolve semantic colors from an Omanix theme colors.toml
# omanix:args=[--file <colors.toml>] (--all | --raw | <key> [fallback])
# omanix:hidden=true

# Shared colors.toml parser/resolver. The alias/fallback cascade is the single
# source of truth every palette consumer (OSC sequences, tmux, claude, pi,
# previews) resolves against, so they all agree on the exact same palette.
#
...
COLORS_FILE="$HOME/.local/state/omanix/current/theme/colors.toml"
OUTPUT=""
QUERY_KEY=""
QUERY_FALLBACK=""

usage() {
  echo "Usage: omanix-theme-color [--file <colors.toml>] (--all | --raw | <key> [fallback])"
}
...
parse_colors_file() {
  ...
  while IFS='=' read -r key value; do
    ...
    if [[ ! $key =~ ^[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-]+$ ]]; then
      printf 'omanix-theme-color: skipping key with unsupported characters\n' >&2
      continue
    fi

    if [[ ! $value =~ ^[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789#(),._+/%\ -]*$ ]]; then
      printf 'omanix-theme-color: skipping %s: unsupported characters in value\n' "$key" >&2
      continue
    fi
