...
# omanix:summary=Enable a shell plugin
# omanix:args=<id> [placement]
# omanix:examples=omanix-plugin-enable acme.weather --section right

set -euo pipefail

fail() {
  echo "omanix-plugin-enable: $*" >&2
  exit 1
}

if [[ ${1:-} == "-h" || ${1:-} == "--help" ]]; then
  echo "Usage: omanix-plugin-enable <id> [section] [--section s] [--index n] [--before id] [--after id]"
  exit 0
fi
...
if (( $# > 0 )) && omanix-plugin-catalog | jq -e --arg id "$id" '
...
result=$(omanix-shell shell enablePlugin "$id" "$placement")
...
elif omanix-plugin-catalog | jq -e --arg id "$id" '
  any(.[]; .id == $id and (.kinds | index("bar")))
' >/dev/null; then
