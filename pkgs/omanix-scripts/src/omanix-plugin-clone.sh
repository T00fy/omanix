...
# omanix:summary=Clone a built-in Omanix shell plugin into your own config
# omanix:args=<source-id> [--edit]
...
copy_plugin() {
  ...
  while IFS=$'\t' read -r source target; do
    ...
    if [[ $source != "$target" ]]; then
      ...
      done < <(grep --recursive --files-with-matches --null --fixed-strings -- "$source" "$target_dir")
    fi
  done < <(jq -r '
    [
      (.entryPoints[] | {source: ., target: .}),
      (.omanix.clonePaths[]? | {source: .source, target: .target})
...
update_manifest() {
  ...
  jq \
    ...
    --arg sourceId "$source_id" '
      ...
      .omanix = (
        (if (.omanix | type) == "object" then .omanix else {} end) +
        { clonedFrom: $sourceId }
      ) |
      del(.omanix.clonePaths)
...
usage() {
  cat <<USAGE
Usage: omanix-plugin-clone <source-id> [--edit]

Copies a built-in plugin into ~/.config/omanix/plugins/<username>.<id>/ so you
...
source_info=$(omanix-plugin-catalog | jq -r --arg id "$source_id" '
...
new_id="${USER:-$(id -un)}.${source_id#omanix.}"
...
for (( attempt = 0; attempt < 40; attempt++ )); do
  if omanix-plugin-list --json | jq -e --arg id "$new_id" 'any(.[]; .id == $id)' >/dev/null; then
    discovered=1
    break
  fi
  sleep 0.05
...
omanix-plugin-enable "$new_id" >/dev/null
clone_complete=1
notify "Editing Cloned Plugin" "Original plugin has been replaced by clone."
