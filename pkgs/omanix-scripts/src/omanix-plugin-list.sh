...
# omanix:summary=List discovered shell plugins
# omanix:args=[--json]
...
while (( $# > 0 )); do
  ...
  --json)
    ...
    ;;
  -h | --help)
    echo "Usage: omanix-plugin-list [--json]"
    exit 0
    ;;
  *)
    echo "omanix-plugin-list: unknown option: $1" >&2
    exit 1
    ;;
  esac
  shift
...
plugins=$(omanix-shell shell listPlugins)
