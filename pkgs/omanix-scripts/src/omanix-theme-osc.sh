...
# omanix:summary=Print OSC sequences for an Omanix color theme
# omanix:hidden=true

theme="${1:-$HOME/.local/state/omanix/current/theme/colors.toml}"
...
while IFS=$'\t' read -r key value; do
  COLORS[$key]="$value"
done < <(omanix-theme-color --file "$theme" --all)
