...
# omanix:summary=Send an omanix desktop notification
# omanix:args=[--app-name <app-name>] [-g <glyph>] [-u <low|normal|critical>] [-i <icon>] [-t <ms>] [-r <id>] [-p] [--image <path-or-uri>] <headline> [description] [--exec <program> [args...]]
# omanix:examples=omanix-notification-send "Reminder" "5 minutes are up" -g 󰢌
...
usage() {
  echo "Usage: omanix-notification-send [--app-name <app-name>] [-g <glyph>] [-u <low|normal|critical>] [-i <icon>] [-t <ms>] [-r <id>] [-p] [--image <path-or-uri>] <headline> [description] [--exec <program> [args...]]" >&2
}
...
# the rest are strings. The click command rides here as omanix-exec-argv, built
# only from --exec below.
hints=(urgency y "$urgency_byte")

if [[ -n $glyph ]]; then
  hints+=(omanix-glyph s "$glyph")
fi
...
if ((exec_present)); then
  ...
  hints+=(omanix-exec-argv s "$exec_argv_json")
fi
...
# omanix-exec-argv is set only from --exec.
#
