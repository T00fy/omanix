...
# omanix:summary=Validate a plugin folder against the Omanix plugin manifest schema
# omanix:args=<plugin-folder>
# omanix:examples=omanix-plugin-validate ./my-plugin
...
fail() {
  echo "omanix-plugin-validate: $*" >&2
  exit 1
}

if [[ ${1:-} == -h || ${1:-} == --help ]]; then
  cat <<USAGE
Usage: omanix-plugin-validate <plugin-folder>
...
[[ $ID != omanix.* ]] || fail "plugin id '$ID' uses the reserved omanix.* namespace"
...
# to open a menu, and so on. Claiming a kind without its entry point leaves a
# plugin that installs, enables, and does nothing. Refuse it here, while there
# is still someone to tell. A kind not listed is left alone rather than guessed.
