...
# omanix:summary=Add a shell plugin from git
# omanix:args=[git-url] [--enable] [--yes]
# omanix:examples=omanix-plugin-add https://github.com/acme/omanix-weather.git --enable
...
PLUGINS_DIR="$HOME/.config/omanix/plugins"
ASSUME_YES=0

fail() {
  echo "omanix-plugin-add: $*" >&2
  exit 1
}
...
plugin_id_manifest() {
  omanix-plugin-catalog | jq -r --arg id "$1" '
...
while (( $# > 0 )); do
  ...
  --yes | -y)
    ...
    ;;
  -h | --help)
    echo "Usage: omanix-plugin-add [git-url] [--enable] [--yes]"
    exit 0
    ;;
  -*)
...
if [[ -z $url ]]; then
  interactive ||
    fail "a git URL is required (e.g. omanix-plugin-add https://github.com/acme/omanix-weather.git)"
...
# enabled. The check is shared with theme install and explains itself; a missing
# checker leaves this non-zero, which refuses the URL rather than cloning it.
omanix-git-url-check "$url" || exit 1

if (( ! ASSUME_YES )); then
  cat >&2 <<WARN

⚠️  Plugins run as arbitrary, unsandboxed code inside your long-lived
  omanix-shell process. Only add repos you trust, and review the code
...
if ! omanix-plugin-validate "$stage"; then
  rm -rf "$stage"
...
if [[ -e $target || -L $target ]]; then
  rm -rf "$stage"
  fail "plugin '$id' is already installed; update it with: omanix-plugin-update $id"
fi

mv "$stage" "$target"
echo "Added $id into $target"

omanix-shell shell rescanPlugins >/dev/null
...
if [[ $enable_after == true ]]; then
  ...
  for (( attempt = 0; attempt < 40; attempt++ )); do
    if omanix-plugin-list --json | jq -e --arg id "$id" 'any(.[]; .id == $id)' >/dev/null; then
      discovered=1
      break
    fi
    sleep 0.05
  ...
  omanix-plugin-enable "$id" "${ENABLE_PLACEMENT[@]}"
else
  echo "Enable it later with: omanix-plugin-enable $id"
fi
