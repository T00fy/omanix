...
# omanix:summary=Pick a screen region over frozen screen content
# omanix:args=[region|windows|smart|fullscreen] [--keep-freeze] [--match-monitor] | --take-fullscreen | --take-window | --select-window <next|prev|left|right|up|down>
# omanix:hidden=true
...
FULLSCREEN_MARKER="${XDG_RUNTIME_DIR:-/tmp}/omanix-capture-region-fullscreen"
WINDOW_MARKER="${XDG_RUNTIME_DIR:-/tmp}/omanix-capture-region-window"
...
# surface (modules/home-manager/desktop/hyprland/bindings.nix) invoke these
# modes. The --take-* modes flag the intent with a marker file and dismiss slurp.
