...
# omanix:summary=Restart audio services and recover stuck USB audio devices.
# omanix:examples=omanix-restart-audio
...
reset_usb_audio_device() {
  ...
  echo "Could not reset $product automatically. Replug or power-cycle it, then run omanix-restart-audio again."
  return 1
}
