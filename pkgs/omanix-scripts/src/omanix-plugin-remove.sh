...
# omanix:summary=Remove an installed shell plugin
# omanix:args=[id] [--yes]
...
while (( $# > 0 )); do
  ...
  --yes | -y)
    ...
    ;;
  -h | --help)
    echo "Usage: omanix-plugin-remove [id] [--yes]"
    exit 0
    ;;
  -*)
...
was_enabled=""
shell_plugins=$(omanix-shell shell listPlugins)
...
if [[ -f $target/manifest.json ]]; then
  cloned_from=$(jq -r '.omanix.clonedFrom // empty' "$target/manifest.json")
fi
...
if [[ $was_enabled == "true" ]]; then
  omanix-shell shell setPluginEnabled "$id" false >/dev/null
fi
...
if [[ -n $cloned_from && $was_enabled == "true" ]]; then
  ...
  echo "Plugin was enabled and was unloaded from omanix-shell."
fi
