...
# Shared helpers for editing ~/.config/omanix/shell.json. Source this from the
# shell-management CLIs; run it directly to print the current resolved config.

CONFIG_FILE="$HOME/.config/omanix/shell.json"
# The "defaults" are the Nix-generated declarative base (quickshell.nix),
# injected as a store path. Empty when the shell module is disabled.
DEFAULTS_FILE="${OMANIX_SHELL_DEFAULTS:-}"
...
# Run directly (not sourced): print the current resolved shell config.
if [[ ${BASH_SOURCE[0]} == "$0" ]]; then
  set -euo pipefail
  src=$(source_file)
  [[ -n $src && -f $src ]] || fail "no shell config found"
  jq -S . "$src"
fi
