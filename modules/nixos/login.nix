...
let
  ...
  # Font is defined in home-manager, but we need it at NixOS level for SDDM
  # Use the same default as modules/home-manager/core/fonts.nix
...
in
{
  options.omanix.login = {
    ...
    font = lib.mkOption {
      type = lib.types.str;
      default = defaultFont;
      description = "Font family for the login screen. Defaults to match omanix.font from home-manager.";
    };
  ...
  config = lib.mkIf (cfg.enable && cfg.login.enable) {
    programs.silentSDDM = {
      ...
      settings = {
        ...
        LockScreen = {
          display = cfg.login.lockScreen.enable;
          background = "${theme.assets.wallpaper}";
          blur = cfg.login.lockScreen.blur;
          brightness = cfg.login.lockScreen.brightness;
          saturation = 0.0;
          padding-top = 50;
          padding-bottom = 50;
        };
        ...
        LoginScreen = {
          background = "${theme.assets.wallpaper}";
          blur = cfg.login.loginScreen.blur;
          brightness = cfg.login.loginScreen.brightness;
          saturation = 0.0;
        };
