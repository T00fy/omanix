...
QtObject {
  ...
  property string pluginsDir: home + "/.config/omanix/plugins"
  ...
  //   - the built-in bar option (`omanix.bar`) is active when `bar.id` is
  //     missing or set to `omanix.bar`.
  ...
  //     silently make `omanix launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "omanix.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.omanix) ? manifest.omanix : null
  ...
  // back, and so a plugin that is both a widget and a menu (omanix.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "omanix.workspaces", center: "omanix.weather", right: "omanix.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.omanix)
        ? candidateManifest.omanix : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "omanix.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "omanix.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `omanix.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("omanix.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party Omanix plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/omanix/plugins.
    var script = ""
