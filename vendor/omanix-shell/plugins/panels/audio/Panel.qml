...
Panel {
  id: root
  moduleName: "omanix.audio"
  ipcTarget: "omanix.audio"

  readonly property var sink: Pipewire.defaultAudioSink
  readonly property var source: Pipewire.defaultAudioSource
  readonly property var nodes: Pipewire.nodes ? Pipewire.nodes.values : []
  readonly property var mprisPlayers: Mpris.players ? Mpris.players.values : []
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("omanix.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("omanix_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // omanix-audio-output-sink resolves the *current* default output through any
  ...
  function setDefaultSource(node) {
    ...
    if (node.id !== undefined && node.name) {
      Quickshell.execDetached([
        "omanix-audio-input-set-default",
        String(node.id),
        String(node.name)
      ])
    }
  ...
  Process {
    ...
    command: ["omanix-audio-sink-availability"]
  ...
  Process {
    id: volumeSinkProc
    command: ["omanix-audio-output-sink"]
    stdout: StdioCollector {
      waitForEnd: true
