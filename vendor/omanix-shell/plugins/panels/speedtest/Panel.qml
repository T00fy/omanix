...
// omanix-network-status when the caller doesn't know it.
Item {
  id: root
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omanix.speedtest")
    else close()
  }
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = ["omanix-network-speedtest", nextPhase]
  ...
  SpeedTestOverlay {
    ...
    layerNamespace: "omanix-network-speedtest"
