...
Panel {
  id: root
  moduleName: "omanix.bluetooth"
  ipcTarget: "omanix.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/omanix-bluetooth-device;
  ...
  function setDefaultAudioSink(sink) {
    ...
    if (sink.id !== undefined && sink.name) {
      Quickshell.execDetached([
        "omanix-audio-output-set-default",
        String(sink.id),
        String(sink.name)
      ])
    }
  ...
  function deviceCommand(action, address) {
    return ["omanix-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via omanix-bluetooth-device.
  ...
  // the adapter came back on at the next boot. omanix-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["omanix-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "omanix.bluetooth"
