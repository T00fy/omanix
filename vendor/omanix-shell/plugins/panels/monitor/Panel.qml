...
Panel {
  id: root
  moduleName: "omanix.monitor"
  ipcTarget: "omanix.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (omanix-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["omanix-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("omanix.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "omanix-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["omanix-display-text-size", String(px)]
  ...
  Process {
    ...
    // `omanix-brightness-display` races the hardware/driver and can
