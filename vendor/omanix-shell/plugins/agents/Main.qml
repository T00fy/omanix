...
// omanix-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/omanix/agents/usage"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["omanix-agent-usage-update"]
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/omanix/agents-disabled.json"
  ...
  // Snapshots keep the field names older Omanix versions wrote, so a fleet
