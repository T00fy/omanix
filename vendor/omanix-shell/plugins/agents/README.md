...
`omanix-agent-usage-update` writes to `~/.local/state/omanix/agents/usage/`
...
`omanix plugin disable omanix.agents`.

## Data

Each agent is one JSON record in `~/.local/state/omanix/agents/usage/`,
written by `omanix-agent-usage-update`. That command runs one
`omanix-agent-usage-<agent>` collector per agent; the widget invokes it
...
configuration in `~/.config/omanix/agents/fireworks.json`:
...
- IPC: `omanix-shell omanix.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/omanix/shell.json`. The
top-level keys can be set with
`omanix bar set omanix.agents <key> <value>`:
...
```bash
omanix bar set omanix.agents refreshIntervalSec 300 --json
omanix bar set omanix.agents syncDir '~/Sync/agent-usage'
```
...
```bash
omanix bar set omanix.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
