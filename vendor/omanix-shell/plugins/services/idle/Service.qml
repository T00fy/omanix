...
Item {
  ...
  // Injected by omanix-shell (the first-party service loader).
  property var shell: null

  readonly property string home: Quickshell.env("HOME")
  readonly property string stayAwakeStateDir: home + "/.local/state/omanix/indicators"
  ...
  function logEvent(event, details) {
    ...
    console.log("omanix idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(omanix-shell lock isLocked 2>/dev/null) == \"true\" ]] || omanix-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "omanix-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "omanix-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/omanix/indicators\" && touch \"$HOME/.local/state/omanix/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/omanix/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/omanix/indicators\"; if [[ -f $HOME/.local/state/omanix/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
