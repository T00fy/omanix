...
function glyphFromHints(hints) {
  return stringHint(hints, "omanix-glyph")
}

// The click action: a JSON argv string from omanix-notification-send
...
function execArgvFromHints(hints) {
  return stringHint(hints, "omanix-exec-argv")
}

// Validate a persisted omanix-exec-argv into a runnable argv, or null. This is
...
// ~/.local/state/omanix/notifications/ so toasts survive shell restarts
// (e.g. the restart `omanix-update` performs). The file exists exactly as
...
// senders (all Omanix web apps) delete their scoped /tmp files on close,
