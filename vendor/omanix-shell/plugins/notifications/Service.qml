// Notification service for the omanix shell.
...
Item {
  ...
  readonly property string stateDir: home + "/.local/state/omanix/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the omanix bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "omanix-notifications"
  ...
  // and rare.
  //   - omanix-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always omanix or system shell scripts —
  ...
  //   - app_name is "omanix-action" (Omanix's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. Omanix's own toasts carry the
  ...
  // toasts survive shell restarts (notably the restart `omanix-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "omanix-notifications"
