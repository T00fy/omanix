...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omanix.emojis")
  }
  ...
  function applySelected(emoji) {
    if (!emoji) return
    root.dismiss()
    Quickshell.execDetached(["omanix-menu-emoji-insert", emoji])
  }

  ListModel { id: displayModel }

  FileView {
    path: root.omanixPath + "/shell/plugins/emojis/emojis.json"
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omanix-emojis"
