...
BarWidget {
  ...
  // UseQApplication` - omanix's shell.qml does not, so every submenu click was
  ...
  function ownedByOmanix(item) {
    var layout = root.bar && root.bar.layoutConfig ? root.bar.layoutConfig : null
    return TrayModel.ownedByOmanix(item, layout)
  }
  ...
  function bucket(category) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (ownedByOmanix(item)) continue
      if (category === "all") {
        result.push(item)
        continue
      }
  ...
  function persistTrayState(pinned, hidden) {
    ...
    var id = root.moduleName || "omanix.tray"
