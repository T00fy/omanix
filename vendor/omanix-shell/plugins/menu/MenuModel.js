...
// against `omanix-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "omanix-channel-current",
  "omanix-default-agent",
  "omanix-default-browser",
  "omanix-default-editor",
  "omanix-default-terminal",
  "omanix-dns"
]
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__omanix_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
