...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omanix.reminders")
  }
  ...
  function submit() {
    ...
    if (root.step === "minutes") {
      ...
      if (!nextMinutes) {
        Quickshell.execDetached(["omanix-notification-send", "Invalid reminder", "Enter the number of minutes"])
        return
      }
    ...
    if (root.step === "message") {
      var args = ["omanix-reminder"].concat(ReminderFlowModel.reminderArgs(root.minutes, selection))
      root.dismiss()
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omanix-reminders"
