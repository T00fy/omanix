...
`omanix-shell` is a single long-running [Quickshell](https://quickshell.org/)
instance that hosts the Omanix desktop. Hyprland autostart launches one shell
...
| `bar`        | A full bar option that can replace the built-in `omanix.bar` |

Only one `bar` plugin is active at a time. Missing or invalid selections fall
back to the built-in `omanix.bar`, so users always have a safe path home.
...
clones it straight into `~/.config/omanix/plugins/<id>/` (named by the
manifest id); updating is a fast-forward pull of that checkout.

```bash
omanix plugin add https://github.com/acme/omarchy-weather.git
omanix plugin update acme.weather       # fetches, shows a diff, fast-forwards
omanix plugin update                    # updates every git-managed plugin
omanix plugin remove acme.weather
```

> ⚠️ **Plugins run as unsandboxed code inside `omanix-shell`.** Adding warns
...
```bash
omanix plugin add https://github.com/acme/omarchy-weather.git --enable --yes
omanix plugin update --yes
```
...
1. Put it in `~/.config/omanix/plugins/<plugin-id>/` with a `manifest.json`
   plus the QML referenced from its `entryPoints`.
2. `omanix-shell shell rescanPlugins`.
3. `omanix plugin enable <id>`. Bar widgets start in
   `barWidget.defaultSection`, or in the center when it is omitted, and can be
   moved with `omanix bar move`; a full bar replaces the one in use.

The lower-level IPC equivalents remain available via `omanix-shell shell rescanPlugins`,
`omanix-shell shell enablePlugin <id> '{}'`, and `omanix-shell shell listPlugins`.
The `omanix plugin` commands wrap those calls. `omanix bar move` and
`omanix bar set` edit the persisted widget layout in `shell.json`.
...
Saving a file anywhere under `~/.config/omanix/plugins/` reloads plugin code
automatically; `omanix-shell shell rescanPlugins` remains available to force a reload.
...
hooks, the image picker's `image-selector` target). `omanix-menu` uses the
shell target to summon the first-party `omanix.menu` plugin instead of
...
| `reloadConfig`                           | `ok`    | reload `~/.config/omanix/shell.json`                 |
...
```
quickshell ipc -p $OMANIX_PATH/shell call shell ping
```

Hyprland autostart launches the shell directly with `quickshell -p
$OMANIX_PATH/shell`. Use `omanix-restart-shell` to stop every running
instance of that config and launch one fresh shell process.

A convenience wrapper, [`omanix-shell`](../bin/omanix-shell), forwards IPC
calls to the running shell. It does not start the shell.

```
omanix-shell shell ping
omanix-shell shell toggle omanix.menu '{"menu":"root"}'
omanix-shell shell listPlugins
omanix-shell shell rescanPlugins
```
...
| Path                              | Owner          | Purpose                                                |
|-----------------------------------|----------------|--------------------------------------------------------|
| `~/.config/omanix/shell.json`    | the shell      | full layout + per-entry settings + enabled plugin list |
| `~/.config/omanix/plugins/<id>/` | user           | drop-in third-party plugin source files                |

The `config/omanix/shell.json` default config describes the
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      "left":   [ { "id": "omanix.menu" }, { "id": "omanix.workspaces" } ],
      "center": [ { "id": "omanix.clock", "format": "HH:mm" } ],
      "right": [
        { "id": "omanix.audio" }
      ]
    }
...
1. **The active bar option is `bar.id`.** Omit it or set it to `omanix.bar`
...
4. **Built-in widget ids are namespaced.** Use ids such as `omanix.clock`,
   `omanix.audio`, and `omanix.network`. The migration rewrites older ids
...
6. **Multiple instances** are allowed when a manifest sets
   ...
   widgets in different timezones are just two `{"id":"omanix.clock", "timezone": ...}`
...
- Phase 1 — `omanix-shell phase 1: host the existing bar in a single shell`
- Phase 2 — `omanix-shell phase 2: plugin registry and bar widget registry`
- Phase 3 — `omanix-shell phase 3: fold bar-settings into the shell as a panel plugin`
- Phase 4 — `omanix-shell phase 4: absorb background-switcher as a plugin`
- Phase 5 — `omanix-shell phase 5: docs, cleanup, and migration crumbs`
- Phase 6 — `omanix-shell phase 6: reviewer cleanup (path traversal, collision, races)`
- Phase 7 — `omanix-shell phase 7: replace socket with IpcHandler, rename to image-picker`
- Phase 8a — `omanix-shell phase 8a: unified shell.json with inline plugin settings`
