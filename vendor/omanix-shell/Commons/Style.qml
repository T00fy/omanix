...
QtObject {
  ...
  // component follows the fontconfig alias `omanix-font-set` writes.
  ...
  // alias path keeps working when the user runs `omanix font set`), but
  ...
  // OMANIX_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("OMANIX_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `omanix font
  ...
  // `omanix toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/omanix/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
