...
QtObject {
  ...
  readonly property string currentThemePath: stateHome + "/omanix/current/theme"
  ...
  // Style. User keys win, so a machine-level `~/.config/omanix/shell.toml`
  ...
  // is where `omanix display text size` writes `[font] base-size`. Watched so the
  // CLI takes effect live without restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/omanix/shell.toml"
    watchChanges: true
    printErrors: false
