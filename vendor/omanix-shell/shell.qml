...
ShellRoot {
  ...
  // The omanix-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. OMANIX_PATH is provided by the uwsm
  ...
  readonly property string shellPath: omanixPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: omanixPath + "/shell/config/shell.json"
  readonly property string userConfigPath: home + "/.config/omanix/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        ...
        center: [{ id: "omanix.clock", format: "dddd HH:mm" }],
        right: [{ id: "omanix.audio" }]
      }
    },
    plugins: []
  })
  ...
  Component.onCompleted: {
    console.log("omanix-shell paths",
      "omanixPath=" + shell.omanixPath,
  ...
  readonly property string defaultBarId: "omanix.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("omanixPath" in target) target.omanixPath = shell.omanixPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("omanixPath" in inst) inst.omanixPath = shell.omanixPath
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. omanix.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("omanixPath" in item) item.omanixPath = shell.omanixPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["omanix.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("omanix.image-picker", payload) ? "ok" : "unknown"
    }
    ...
    function cancel(doneFile: string): string {
      ...
      if (picker && typeof picker.closeSelector === "function") {
        ...
        shell.hide("omanix.image-picker")
      }
      return "ok"
    }
  ...
  IpcHandler {
    ...
    function listPlugins(): string {
      ...
      for (var id in plugins) {
        ...
        var metadata = plugins[id].omanix
        var clonedFrom = Util.isPlainObject(metadata) ? String(metadata.clonedFrom || "") : ""
        out.push({
          id: id,
          name: plugins[id].name,
          kinds: kinds,
          // What `omanix plugin enable/disable` toggles: for a widget that is
