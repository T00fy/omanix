...
- [x] ✅ **Audio tuning** (Q4-03): shipped the tuning framework `omanix-audio-tuning` (`on|off|status|match|fronted-sink`), the always-on shell-contract helpers `omanix-audio-{output-sink,sink-availability,output-set-default,input-set-default,output-volume,source-switch}` + `omanix-brightness`, and `omanix-restart-audio` (stuck-USB recovery via `usbreset`), plus reworked `omanix-cmd-audio-switch` off swayosd. Tuning data ships read-only in a new `pkgs/omanix-audio-tunings/` package (`dell-xps-2026`: 13-biquad EQ + LSP LV2 limiter, `@SPEAKER_SINK@` runtime-substituted), path injected via `OMANIX_AUDIO_TUNINGS` (mirrors the `OMANIX_THEMES_DIR` bridge). The filter-chain runs as its **own PipeWire client** in a Nix-declared on-demand `systemd.user.services.omanix-speaker-tuning` (`WantedBy=[]`, `pipewire -c`), gated behind new `omanix.audio.speakerTuning.enable` (default false) — `lsp-plugins` and the LV2 path are pulled into the closure **only** when enabled; toggling the tuning needs no audio restart. Sink helpers always ship (shell calls them unconditionally) and resolve loudness through any DSP/virtual sink to the physical one. **Media-key rewire (deferred from Q3-01, full removal):** every media key moved off `swayosd-client` — volume/mute/mic → `omanix-audio-output-volume`, brightness → `omanix-brightness`, playerctl → `playerctl` direct (shell Mpris/OSD surfaces feedback); `ui/swayosd.nix` and swayosd deleted everywhere. rtkit/realtime left to the host per the "audio is host-provided" stance. `nix build` (scripts + tunings) + `nix flake check` pass; runtime behaviour verifies on a live build.
- [x] ✅ **Network / DNS / Bluetooth / Power** (Q4-04): shipped all ten runtime helpers the network, bluetooth, and power panels shell out to (reconstructed from each panel's `Model.js`/`Panel.qml` call contract — upstream `bin/` was never vendored). **Widened scope** past this line's five `omanix-network-*` to also fold in `omanix-dns`, `omanix-bluetooth-{device,power}`, and `omanix-powerprofiles-{list,set}` — the line-214 Q1-13 follow-ups for those same three panels, which no other Q4 ticket owned. These are all live device-mutation CLIs (NM connections, rfkill, power profiles) Nix cannot declare, so they belong in `pkgs/omanix-scripts/` (same reasoning as Q4-02/Q4-03). `omanix-network-status` is dual-mode (bare line for the speedtest panel, `--verbose` key/value for the network panel, parallel router+internet ping); `omanix-network-band` queries/pins the Wi-Fi band and reverts on failed reassociation; `omanix-network-qr` emits a square 0/1 matrix via `qrencode` and refuses 802.1X; `omanix-network-speedtest` streams Mbps from fast.com workers sampling sysfs counters; `omanix-dns` maps DHCP/Cloudflare/Google/Custom on the active connection. `omanix-bluetooth-power` moves the rfkill soft block (persists across reboot) not just BlueZ `Powered`. `omanix-powerprofiles-*` degrade to a no-op when power-profiles-daemon is absent (leaves a TLP host alone). **802.1X needs no script** — enterprise EAP connect is fully inline in the vendored `network/Model.js` (`enterpriseConnectScript`: raw `nmcli`+`uuidgen`, password over stdin); it only needs `nmcli`/`uuidgen` on the session PATH (runtime verify). **Nix-idiomatic daemon posture — no new module/option:** NetworkManager stays host-provided (Q1-13), Bluetooth is already enabled (`modules/nixos/default.nix:470`), power-profiles-daemon is the NixOS default; scripts get their binaries via `deps`, daemons via defaults. New nixpkgs deps: `networkmanager`, `iw`, `qrencode`, `curl`, `iproute2`, `iputils`, `bluez`, `power-profiles-daemon`. `nix build .#omanix-scripts` + `nix flake check` pass; live panel/802.1X behaviour verifies on a real build.
- [x] ✅ **Capture** (Q4-05): shipped the five testable capture primitives as `omanix-*` CLIs (ported near-verbatim from omarchy `bin/` — upstream `bin/` was never vendored — pinned rev `v4.0.2` `346e69e1`, D1 rename applied): `omanix-capture-qr` (hyprpicker freeze → slurp → grim → `zbarimg`, `wl-copy --sensitive`, never prints to stdout), `omanix-capture-text` (region OCR via `tesseract`, `OMANIX_OCR_LANGS`), `omanix-capture-region` (the keyboard-navigable shared picker: `--take-{fullscreen,window}`/`--select-window <dir>` + `region|windows|smart|fullscreen`, markers under `$XDG_RUNTIME_DIR`), `omanix-capture-webcam-list` (`v4l2-ctl` walk — unblocks Q4-02's `omanix-hw-webcam`, whose sysfs fallback + TODO were trimmed), and `omanix-transcode` (magick/ffmpeg → clipboard uri-list; omarchy's `omarchy-menu-{file,select}` don't exist here, so file/format/resolution selection was reconstructed over `omanix-menu-dmenu`). New scripts deps: `zbar`, `tesseract`, `ffmpeg`, `file`, `v4l-utils`. **Wiring:** fixed the stale screen-recording bar indicator (`vendor/…/ScreenRecording.qml` retargeted at omanix's `wl-screenrec` recorder; logged in `vendor/PROVENANCE.md`), added `trigger.capture.{qr,text,transcode}` menu entries, and added the omarchy keybinds — static `SUPER+CTRL+PRINT` (OCR) / `SUPER+CTRL+PERIOD` (transcode) plus the dynamic slurp-layer window-selection binds (RETURN/CTRL+RETURN/TAB/CTRL+TAB/arrows) injected as raw Lua via `wayland.windowManager.hyprland.extraConfig` (scoped to the `selection` layer). **Scope call — skipped the two webcam-*recording* tools** (`omanix-capture-webcam-resize`, `omanix-capture-screenrecording-with-webcam`): omanix intentionally uses `wl-screenrec` (not omarchy's gpu-screen-recorder) and dropped the webcam overlay, so the feature has no backing here and is untestable. `nix build` (omanix-scripts + omanix-shell) + `nix flake check` pass; QR/OCR/region/transcode/indicator behaviour verifies on a live Hyprland session.
- [x] ⏭️ **Security** (Q4-06): skipped — sshd hardening + docker-group posture are stock NixOS one-liners (`services.openssh`, `networking.firewall`, `users.users.<u>.{openssh.authorizedKeys,extraGroups}`); wrapping them adds no value and the `githubKeys` build-time fetch is anti-idiomatic. Intent served by a "Security (SSH & Docker)" section in `docs/configuration.md` (D-philosophy: point at stock options, don't wrap install wizards). Consistent with §1 "install/provisioning … Nix obsoletes."
- [x] ✅ **Tailscale taildrop** (Q4-07): shipped `omanix-tailscale-send` (`tailscale file cp`; the shell panel calls it with just a peer, so a no-file invocation opens the new floating-terminal picker `omanix-file-select` — ported for omarchy's `omarchy-file-select`, reuses the `org.omanix.terminal` floating rule + `fzf`) and `omanix-tailscale-receive` (long-running `tailscale file get --wait --conflict=rename` into `~/Downloads/.omanix-taildrop`, atomic `ln`/link(2) claim into `~/Downloads` with rename-on-collision, per-file `notify-send` with image preview + best-effort click-to-open; `--once` one-shot). New deps: `tailscale`. Receiver runs as a Nix `systemd.user.services.omanix-tailscale-receive` in new HM module `desktop/tailscale.nix`, gated on `omanix.tailscale.taildrop.enable` (default false, `Restart=on-failure`, `WantedBy=graphical-session.target`); it references the wrapped binary by absolute path via a new internal `omanix.scripts.package` readOnly option (user services don't inherit the interactive PATH). **Q0-05 `pkexec` landmine resolved:** neutralized `authorizeTailscaleOperator()` in vendored `tailscale/Service.qml` to an inert no-op (logged in `vendor/PROVENANCE.md`); the operator is instead declared via new NixOS `omanix.tailscale.operator` (a `tailscaled` oneshot in `modules/nixos/tailscale.nix`, gated on `services.tailscale.enable` — does not enable tailscale itself). Tailscale enablement stays host-provided. `nix build .#omanix-scripts` + `nix flake check` pass; both options appear in the generated docs. Send/receive/picker behaviour is runtime-only to verify.
