...
- [ ] Port `omanix-theme-switcher` (image picker) + background helpers (`omanix-theme-bg-{switcher,set,cache,current}`) — Q2-05. Note: the vendored `Background.qml` invokes `omanix-theme-bg-set` (not `omanix-theme-bg-current`) on the wallpaper-switcher double-click; ship `bg-set` to satisfy that contract.
